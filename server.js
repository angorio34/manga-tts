import express from "express"; import cors from "cors"; import fetch from "node-fetch"; import { createWriteStream, promises as fs } from "fs"; import { exec } from "child_process";
...
try {
    // Загружаем изображение страницы манги
    ...
    exec("tesseract page.jpg output", async (error) => {
        ...
        try {
            ...
            res.status(500).json({ error: "Ошибка чтения файла output.txt" });
        }
    });
} catch (err) {
